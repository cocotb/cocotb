...
def _start_user_coverage() -> None:
    ...
    if env.exists("COCOTB_USER_COVERAGE"):
        enable_coverage = env.as_bool("COCOTB_USER_COVERAGE")

    elif env.exists("COVERAGE"):
        warnings.warn(
            "COVERAGE is deprecated in favor of COCOTB_USER_COVERAGE",
            DeprecationWarning,
            stacklevel=2,
        )
        enable_coverage = env.as_bool("COVERAGE")
    ...
    if enable_coverage:
        try:
            ...
            config_filepath: str = env.as_str("COVERAGE_RCFILE")
...
def _setup_random_seed() -> None:
    ...
    if env.exists("COCOTB_RANDOM_SEED"):
        seed = env.as_int("COCOTB_RANDOM_SEED", seed)

    elif env.exists("RANDOM_SEED"):
        warnings.warn(
            "RANDOM_SEED is deprecated in favor of COCOTB_RANDOM_SEED",
            DeprecationWarning,
        )
        seed = env.as_int("RANDOM_SEED", seed)
...
def _setup_regression_manager() -> None:
    ...
    # discover tests
    modules: list[str] = env.as_list("COCOTB_TEST_MODULES")
    if not modules:
        raise RuntimeError(
    ...
    # filter tests
    testcases: list[str] = env.as_list("COCOTB_TESTCASE")
    test_filter: str = env.as_str("COCOTB_TEST_FILTER")
