...
GpiIterator::Status VpiRootIterator::next_handle(std::string &, GpiObjHdl **hdl,
    ...
    vpiHandle root;
    while ((root = vpi_scan(m_iterator)) != NULL) {
        check_vpi_error();
...
GpiIterator::Status VpiIterator::next_handle(std::string &name, GpiObjHdl **hdl,
    ...
    do {
        ...
        if (m_iterator) {
             ...
             * match the name of the generate block. Arrays of interface
             * instances and of interface references (vpiRefObj) are handled
             * the same way, see get_child_by_name().
             */
