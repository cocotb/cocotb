...
void gpi_start_of_sim_time(int argc, char const *const *argv) {
    for (auto it = start_of_sim_time_cbs.rbegin();
         it != start_of_sim_time_cbs.rend(); it++) {
        // start_of_sime_time should never fail, this should be moved to
        // gpi_load_users, as should the (argc,argv)
        if (it->first(it->second, argc, argv)) {
            gpi_end_of_sim_time();
        }
    }
}
...
void gpi_end_of_sim_time() {
    for (auto it = end_of_sim_time_cbs.rbegin();
         it != end_of_sim_time_cbs.rend(); it++) {
        it->first(it->second);
    }
