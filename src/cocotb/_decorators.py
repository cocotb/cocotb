...
from cocotb._py_compat import TypeAlias, cached_property
...
class Test:
    ...
    Args:
        ...
        kwargs:
            Keyword argument to pass to the test function.
    ...
    **options_by_name: Sequence[object],
    ...
    def wrapper(f: TestFuncType | TestGenerator) -> TestGenerator:
        if not isinstance(f, TestGenerator):
            f = TestGenerator(f)
        # Ensure we prepend so that arguments lexically father down in the "stack" appear
        # in the test name lexically after (more right).
