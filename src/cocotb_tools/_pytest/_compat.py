...
from pytest import Class, Collector, Config, Item, Mark, Module, fail, hookimpl, mark
...
def cocotb_decorators_to_pytest_marks(test: Test | TestGenerator) -> object:
    ...
    if test.stage:
        ...
        try:
            ...
            if "not found" in str(e):
                fail(
                    reason="The 'order' fixture is not present. Did you forget to install the 'pytest-order' plugin or similar?"
                )
            else:
                raise e from None
    ...
    # Add the @pytest.mark.cocotb marker. Plugin is expecting this as a proper cocotb test
