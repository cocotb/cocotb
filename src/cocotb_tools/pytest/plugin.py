...
from cocotb_tools.pytest._collector import Cocotb
from cocotb_tools.pytest._compat import Compat
from cocotb_tools.pytest._handle import MockSimHandle
from cocotb_tools.pytest._option import Option, populate_ini_options
from cocotb_tools.pytest._reporter import Reporter
from cocotb_tools.pytest._results import ResultsXML
...
_OPTIONS: tuple[Option, ...] = (
    ...
    Option(
        ...
        action="store_true",
        description="""
            Disable managing building HDL designs and running HDL simulations with cocotb tests based on defined
            :class:`~cocotb_tools.pytest.dut.Dut` fixtures and `@pytest.mark.cocotb_*` markers.
            This option is useful when explicitly using :class:`~cocotb_tools.runner.Runner` in separate test functions
            to build HDL designs and run HDL simulations with cocotb tests.
        """,
    ),
    Option(
        "gpi_log_level",
...
@fixture
def dut(request: FixtureRequest) -> Dut:
...
def pytest_configure(config: Config) -> None:
    ...
    if not cocotb.is_simulation:
        cocotb.top = cast("SimHandleBase", MockSimHandle())
        cocotb.SIM_NAME = option.cocotb_simulator
        cocotb.SIM_VERSION = "0.0.0"

        config.pluginmanager.register(Reporter(config))

        if option.cocotb_regression_manager == "cocotb" and not option.collectonly:
            config.pluginmanager.register(ResultsXML(config))
        ...
        # Populate environment variables for simulation processes
        os.environ["COCOTB_PYTEST_DIR"] = str(Path(invocation_dir).resolve())
        os.environ["COCOTB_PYTEST_ARGS"] = shlex.join(invocation_args)

    config.pluginmanager.register(Compat(config))


def pytest_sessionstart(session: Session) -> None:
    """Initialize the session, registering the top-level Cocotb collector.

    This hook is called after the :class:`pytest.Session` object has been created
    and before performing collection and entering the test run loop.
    ...
    if not cocotb.is_simulation:
        # Collect information about HDL designs from Dut fixtures and @pytest.mark.cocotb_* markers
        config.pluginmanager.register(Cocotb.from_parent(session))
