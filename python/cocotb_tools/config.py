...
Global variables:
    ...
    cmake_dir: str, path where the cocotb CMake package is installed
    verilator_cpp: str, path to cocotb's Verilator runtime source
"""
...
source_root = base_tools_dir.parents[1]
source_gpi_dir = source_root.joinpath("cpp", "gpi")
if source_gpi_dir.is_dir():
    share_dir = source_gpi_dir
    cmake_dir = source_root.joinpath("build", "cmake", "cocotb")
else:
    share_dir = base_cocotb_dir.joinpath("share")
    cmake_dir = share_dir.joinpath("cmake", "cocotb")
libs_dir = base_cocotb_dir.joinpath("libs")
makefiles_dir = base_tools_dir.joinpath("makefiles")
verilator_cpp = base_tools_dir.joinpath("data", "verilator", "verilator.cpp")
...
def _get_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument(
        "--cmakedir",
        action="store_true",
        help="Print the path to cocotb's CMake package directory",
    )
    group.add_argument(
        "--verilator-cpp",
        action="store_true",
        help="Print the path to cocotb's Verilator runtime source",
    )
    group.add_argument(
        "--python-bin",
        action="store_true",
...
def main() -> None:
    ...
    if args.share:
        ...
        print(cmake_dir.as_posix())
    elif args.verilator_cpp:
        print(verilator_cpp.as_posix())
