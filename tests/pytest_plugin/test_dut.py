...
def test_dut_suffix(pytester: Pytester) -> None:
    """Test setting the suffix name of DUT fixture."""
    pytester.makepyfile("""
        import pytest
        from cocotb_tools.pytest.dut import Dut

        @pytest.mark.cocotb_suffix("with_timescale_1ns")
        def test_suffix(dut: Dut) -> None:
            assert dut.name.endswith(".with_timescale_1ns")
            assert dut.suffix == "with_timescale_1ns"
    """)
