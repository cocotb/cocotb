...
def test_pytest_plugin_regression_manager_with_basic_module(
    pytester: Pytester, collect_only: bool
) -> None:
    ...
    pytester.makepyfile(
        ...
        async def async_generator_fixture(dut) -> AsyncGenerator[SimHandleBase, None]:
            # Setup
            await simple_transfer(dut)
        ...
        async def coroutine_fixture(dut) -> SimHandleBase:
            await simple_transfer(dut)
        ...
        @pytest.mark.cocotb
        async def test_with_async_generator_fixture(dut, async_generator) -> None:
            await simple_transfer(async_generator)

        @pytest.mark.cocotb
        async def test_with_coroutine_fixture(dut, coroutine) -> None:
            await simple_transfer(coroutine)

        @pytest.mark.cocotb
        async def test_value_from_async_generator_fixture(dut, value_async_generator: int) -> None:
            assert value_async_generator == 1234

        @pytest.mark.cocotb
        async def test_value_from_coroutine_fixture(dut, value_coroutine: int) -> None:
            assert value_coroutine == 1234

        @pytest.mark.cocotb
        @pytest.mark.xfail(raises=RuntimeError)
        async def test_with_failed_setup_async_generator_fixture(dut, failed_setup_async_generator: int) -> None:
            assert failed_setup_async_generator == 1234

        @pytest.mark.cocotb
        @pytest.mark.xfail(raises=RuntimeError)
        async def test_with_failed_teardown_async_generator_fixture(dut, failed_teardown_async_generator: int) -> None:
            assert failed_teardown_async_generator == 1234

        @pytest.mark.cocotb
        @pytest.mark.xfail(raises=RuntimeError)
        async def test_with_failed_coroutine_fixture(dut, failed_coroutine: int) -> None:
...
@pytest.mark.simulator_required
@pytest.mark.parametrize("collect_only", (True, False))
def test_pytest_plugin_regression_manager_with_sample_module(
    pytester: Pytester,
    collect_only: bool,
    designs_dir: Path,
    ...
    pytester.makeconftest(
        ...
        @pytest.fixture(name="dut", scope="module")
        def dut_fixture(runner: FixtureRunner) -> Callable[[], object]:
            ...
            if runner.option.cocotb_simulator == "questa":
                build_args += ["+acc"]
                sim_args += ["-t", "ps"]

            elif runner.option.cocotb_simulator == "xcelium":
                build_args += ["-v93"]

            elif runner.option.cocotb_simulator == "nvc":
                build_args += ["--std=08"]

            if runner.option.cocotb_simulator not in ("xcelium",):
            ...
            return lambda: runner.test(
                test_module=test_modules,
                hdl_toplevel="sample_module",
                test_args=sim_args,
                timescale=timescale,
                gpi_interfaces=runner.option.cocotb_gpi_interfaces or None,
            )
        '''
    )
