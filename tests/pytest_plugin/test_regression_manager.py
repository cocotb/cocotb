...
def test_pytest_plugin_regression_manager_with_basic_module(
    ...
    if regression_manager == "pytest":
        if with_user_runners:
            # It includes explicit test function with user runner
            tests = 40
            passed = 30
            skipped = 2
            xfailed = 7
            ...
            # It doesn't include implicit test function created by plugin
            tests = 39
            passed = 29
            skipped = 2
            xfailed = 7
        ...
        # TODO: To get proper number of cocotb tests from all test modules it will require to send TestReport to pytest via IPC
        if with_user_runners:
            # It includes explicit test function with user runner
            tests = 32
            passed = 1
    ...
    if with_user_runners:
        pytester.makepyfile(
            ...
            def test_runner(request: FixtureRequest) -> None:
                ...
                language: str = option.cocotb_toplevel_lang or list(runner.supported_gpi_interfaces)[0]
                sources: list[Path] = []
        ...
        pytester.makeconftest(
            ...
            def dut_fixture(runner: FixtureRunner) -> Union[FixtureRunner, SimHandleBase]:
