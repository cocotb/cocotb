...
def test_mark_cocotb_simulation() -> None:
    """Test signature of :deco:`cocotb_tools.pytest.mark.cocotb_simulation` marker."""
    marker: MarkDecorator = mark.cocotb_simulation()
    assert len(marker.args) == 0
    assert len(marker.kwargs) == 0
    assert marker.__doc__


def test_mark_cocotb_test() -> None:
    """Test signature of :deco:`cocotb_tools.pytest.mark.cocotb_test` marker."""
    marker: MarkDecorator = mark.cocotb_test()
    assert len(marker.args) == 0
    assert len(marker.kwargs) == 0
    assert marker.__doc__


def test_mark_cocotb_suffix() -> None:
    """Test signature of :deco:`cocotb_tools.pytest.mark.cocotb_suffix` marker."""
    marker: MarkDecorator = mark.cocotb_suffix("with_timescale_1ns")
    assert len(marker.args) == 1
    assert len(marker.kwargs) == 0
    assert marker.args[0] == "with_timescale_1ns"
    assert marker.__doc__
