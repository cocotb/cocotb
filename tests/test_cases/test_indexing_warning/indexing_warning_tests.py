...
async def test_indexing_warnings(dut: Any) -> None:
    with pytest.warns(IndexingChangedWarning, match="Update index 0 to 7"):
        dut.vec.value[0]
    with (  # 3:7 is TO, but the HDL 7:0 is DOWNTO
        pytest.raises(IndexError),
        pytest.warns(IndexingChangedWarning, match="Update slice 3:7 to 4:0"),
    ):
        dut.vec.value[3:7]

    with (  # 0 is out of range of the HDL's 1:4
        pytest.raises(IndexError),
        pytest.warns(IndexingChangedWarning, match="Update index 0 to 1"),
    ):
        dut.arr.value[0]
