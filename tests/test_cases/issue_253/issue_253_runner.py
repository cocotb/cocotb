...
def test_issue_253_runner():
    """Test issue_253 using the Python runner."""
    ...
    runner.test(
        hdl_toplevel="sample_module",
        hdl_toplevel_lang=toplevel_lang,
        test_module="COCOTB_TEST_FILTER=issue_253_none  	vvp -M $(shell cocotb-config --lib-dir) -m $(shell cocotb-config --lib-name vpi icarus) $(SIM_BUILD)/sim.vvp $(SIM_ARGS) $(EXTRA_ARGS) $(PLUSARGS)",
        test_args=test_args,
    )


if __name__ == "__main__":
    test_issue_253_runner()
