...
import cocotb
from cocotb._sim_versions import NvcVersion
from cocotb.handle import HierarchyObject, IntegerObject, LogicObject, StringObject
...
nvc_pre_1_13 = SIM_NAME.startswith("nvc") and (
    NvcVersion(cocotb.SIM_VERSION) < NvcVersion("1.13")
)
...
async def test_get_root_handle(dut):
    ...
    handle = cocotb.simulator.get_root_handle("cocotb_package_pkg_1")
    assert handle is not None
    pkg1 = cocotb.handle.SimHandle(handle)
    assert pkg1.five_int.value == 5
