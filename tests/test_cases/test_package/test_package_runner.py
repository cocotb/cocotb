...
def test_test_package_runner():
    """Test test_package using the Python runner."""
    ...
    if toplevel_lang == "verilog":
        sources = [
            test_dir / "test_cases/test_package/cocotb_package_pkg.sv",
            test_dir / "test_cases/test_package/cocotb_package.sv",
        ]
    else:
    ...
    runner.test(
        hdl_toplevel="cocotb_package",
        hdl_toplevel_lang=toplevel_lang,
        test_module="test_package",
        test_args=test_args,
    )


if __name__ == "__main__":
    test_test_package_runner()
