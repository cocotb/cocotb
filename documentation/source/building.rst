...
.. make:var:: COCOTB_HDL_TIMEUNIT
      ...
      The default time unit that should be assumed for simulation when not specified by modules in the design. If this isn't specified then it is assumed to be ``1ns``. Allowed units are ``s``, ``ms``, ``us``, ``ns``, ``ps``, ``fs``.
