...
.. glossary::
   ...
   deposit
      Set the value of a signal or variable once. May be :term:`inertial <inertial deposit>` or :term:`no-delay <no-delay deposit>`.

   force
      Set the value to a signal or variable once immediately, then locking it so the value can't change.

   release
      Set the value to a signal or variable once immediately, then unlocking it from a previous :term:`force`.
