...
.. warning::
    Calling :meth:`!result` or :meth:`!exception` on a cancelled task will raise the :class:`~asyncio.CancelledError` exception.
...
These functions allow you to wait for multiple :term:`awaitables <awaitable>`, including :term:`coroutines <coroutine>` and :class:`~cocotb.task.Task`\ s, simulateously.
...
.. code-block:: python
    ...
    async with TaskManager() as tm:
        saw_complete = tm.start_soon(RisingEdge(cocotb.top.operation_complete))
        ...
        await do_work
        if not saw_complete.done():
            raise TimeoutError("Operation did not complete within 1 us!")
