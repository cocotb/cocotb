...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /**
   * Uncollapse all parent TOC elements of the current link.
   * @param {HTMLElement} link - The currently active TOC link.
   */
  function uncollapseParents(link) {
    let currentItem = link.closest('li');

    while (currentItem) {
      // Find the closest parent <ul> and its parent <li>
      const parentUl = currentItem.parentElement;
      const parentLi = parentUl.closest('li');

      if (parentLi) {
        // Add a class or remove 'collapsed' if using a collapsible TOC
        parentLi.classList.remove('collapsed');

        // If there's a button or element controlling collapse, simulate a click or set state
