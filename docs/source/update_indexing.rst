...
cocotb 2.0 changed what types were returned when getting a value from a
:ref:`Logic-like <logic-handle-value-changes>` or :ref:`Array-like <array-handle-value-changes>` simulator object.
This can require a change in how those values are indexed.
...
.. code-block:: python
    ...
    assert dut.array.value[0] == 1  # index 0 is always left-most

.. code-block:: python
    :caption: New HDL-based indexing
    :class: new
    ...
    assert dut.array.value[1] == 1  # index 1 is now the left-most
...
If you are looping over the indexes to then index each element, you can instead iterate over the array directly.
...
Prefer this as a last resort, as it is less readable and has overhead compared to the other options.
...
The recommended approach to fix this change is to update one module at a time.
This is done by using the :envvar:`PYTHONWARNINGS` environment variable to limit which modules can emit a :class:`!IndexingChangedWarning`.
