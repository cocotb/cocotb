...
- Added the :external+cocotb20:attr:`.ValueObjectBase.is_const` property to support checking if a :class:`ValueObject` is immutable. (:pr:`2720`)
...
- Added :external+cocotb20:func:`cocotb.parametrize`, a decorator that serves as an alternative to :external+cocotb20:class:`~cocotb.regression.TestFactory`. (:pr:`3513`)
...
- The :external+cocotb20:func:`cocotb.test` decorator now accepts a ``name`` argument to override the name of the test in the :external+cocotb20:class:`~cocotb.regression.RegressionManager`. (:pr:`3578`)
...
- Added :external+cocotb20:meth:`.HierarchyObject._keys`, :external+cocotb20:meth:`.HierarchyObject._values`, and :external+cocotb20:meth:`.HierarchyObject._items` to help users dynamically interact with hierarchical elements of the :term:`DUT`. (:pr:`3655`)
...
- Use parameter values in generated test names of :external+cocotb20:func:`cocotb.parametrize`, which should be clearer and allow the user to better group tests using :external+cocotb20:envvar:`COCOTB_TEST_FILTER`. (:pr:`3717`)
- Support specifying arguments like in :external+cocotb20:meth:`.TestFactory.add_option` in :external+cocotb20:func:`cocotb.parametrize`. (:pr:`3717`)
- Added :external+cocotb20:attr:`~cocotb.handle.LogicArrayObject.range`, :external+cocotb20:attr:`~cocotb.handle.LogicArrayObject.left`, :external+cocotb20:attr:`~cocotb.handle.LogicArrayObject.direction`, and :external+cocotb20:attr:`~cocotb.handle.LogicArrayObject.right` properties to :external+cocotb20:class:`~cocotb.handle.LogicArrayObject`. (:pr:`3733`)
- Added :external+cocotb20:attr:`~cocotb.handle.ArrayObject.range`, :external+cocotb20:attr:`~cocotb.handle.ArrayObject.left`, :external+cocotb20:attr:`~cocotb.handle.ArrayObject.direction`, and :external+cocotb20:attr:`~cocotb.handle.ArrayObject.right` properties to :external+cocotb20:class:`~cocotb.handle.ArrayObject`, :external+cocotb20:class:`~cocotb.handle.LogicArrayObject`, and :external+cocotb20:class:`~cocotb.handle.StringObject`. (:pr:`3733`)
- Added :external+cocotb20:attr:`~cocotb.handle.StringObject.range`, :external+cocotb20:attr:`~cocotb.handle.StringObject.left`, :external+cocotb20:attr:`~cocotb.handle.StringObject.direction`, and :external+cocotb20:attr:`~cocotb.handle.StringObject.right` properties to :external+cocotb20:class:`~cocotb.handle.StringObject`. (:pr:`3733`)
- Added support for using :class:`str` in assignment to :external+cocotb20:class:`~cocotb.handle.LogicObject`\ s. (:pr:`3733`)
- Setting a value with :external+cocotb20:attr:`ArrayObject.value <cocotb.handle.ArrayObject.value>` now accepts any :class:`~collections.abc.Sequence`\ -like type, such as :class:`tuple` and :class:`list`, as well as :external+cocotb20:class:`~cocotb.types.Array`. (:pr:`3733`)
...
- Introduced :external+cocotb20:attr:`cocotb.is_simulation` which is ``True`` only when the cocotb library was loaded in a simulation. (:pr:`3779`)
- The :ref:`combine_results.py <combine-results>` script now ships with the cocotb installation. (:pr:`3791`)
- Added :external+cocotb20:meth:`.LogicArray.to_unsigned` and :external+cocotb20:meth:`.LogicArray.to_signed` to convert :external+cocotb20:class:`~cocotb.types.LogicArray` into :class:`int`. (:pr:`3792`)
- Added :external+cocotb20:meth:`.LogicArray.from_unsigned` and :external+cocotb20:meth:`.LogicArray.from_signed` to construct :external+cocotb20:class:`~cocotb.types.LogicArray` from :class:`int`. (:pr:`3792`)
...
- Added :external+cocotb20:class:`cocotb.simulator.cpp_clock`, a C++ clock generator implementation with higher performance due to less handshaking between Python and the GPI. :external+cocotb20:class:`~cocotb.clock.Clock` uses it automatically when it would behave identically to the Python implementation. (:pr:`3983`)
- The :ref:`Siemens DSim <sim-dsim>` simulator is now supported by cocotb. (:pr:`3990`)
- Added :external+cocotb20:meth:`.LogicArray.to_bytes` and :external+cocotb20:meth:`.LogicArray.from_bytes` for converting :external+cocotb20:class:`~cocotb.types.LogicArray` to and from :class:`bytes`. (:pr:`4098`)
...
- Allow user to specify elaboration arguments (``elab_args``) to :external+cocotb20:meth:`.Runner.test` for :ref:`NVC <sim-nvc>` (e.g. to enable coverage collection). (:pr:`4267`)
- Added :external+cocotb20:attr:`cocotb.types.logic_array.RESOLVE_X` which controls the default resolution behavior of ``X``, ``Z``, ``U``, ``W``, and ``-`` values in :external+cocotb20:class:`.LogicArray`\ s whenever they are converted to integers using :class:`int` casts, :external+cocotb20:meth:`.LogicArray.to_unsigned` or :external+cocotb20:meth:`.LogicArray.to_signed` without passing an argument for the ``resolve`` parameter. (:pr:`4298`)
- Adding support for resolving ``X``, ``Z``, ``U``, ``W``, and ``-`` values to :external+cocotb20:meth:`.LogicArray.to_unsigned` and :external+cocotb20:meth:`.LogicArray.to_signed`. (:pr:`4298`)
- :external+cocotb20:envvar:`PYGPI_USERS` supports multiple user modules by comma-separating them. (:pr:`4310`)
- The Makefile flow no longer requires that :external+cocotb20:ref:`cocotb-config <cocotb-config>` be located on the ``PATH``. Set :external+cocotb20:envvar:`PYTHON_BIN` if the wrong Python executable is used. (:pr:`4338`)
- Added :external+cocotb20:class:`.TaskComplete` Trigger and :external+cocotb20:attr:`.Task.complete` to get that Trigger for the Task. These are intended to replace :external+cocotb20:class:`.Join` and :external+cocotb20:meth:`.Task.join` and do not return the Tasks result when :keyword:`await`\ ed. (:pr:`4341`)
...
- :external+cocotb20:class:`~cocotb.clock.Clock` now owns the driver task, so the user does not need to pass the result of :external+cocotb20:meth:`.Clock.start` to :external+cocotb20:func:`cocotb.start_soon`. Additionally, :external+cocotb20:meth:`.Clock.stop` was introduced to stop the clock driver. (:pr:`4396`)
- Add :external+cocotb20:meth:`.Clock.cycles` to wait a given number of clock cycles. (:pr:`4396`)
...
- Guarantee that :external+cocotb20:class:`.Lock` acquisition is fair. (:pr:`4473`)
...
- Added :meth:`~.HierarchyObject._get` which returns ``None`` on failure instead of raising an exception to aid with optional signal discovery. It also accepts the optional parameter ``discovery_method`` that takes :external+cocotb20:class:`~cocotb.handle.GPIDiscovery` to specialize object discovery. (:pr:`4517`)
- Added :external+cocotb20:data:`cocotb.triggers.current_gpi_trigger` to allow the user to determine the last GPI trigger that fired. (:pr:`4549`)
...
- Added :external+cocotb20:meth:`.Logic.resolve` and :external+cocotb20:meth:`.LogicArray.resolve` to resolve non-``0``/``1`` values on demand. (:pr:`4622`)
- :external+cocotb20:class:`.LogicArray` now supports :class:`bool` casts and usage in conditionals (e.g. ``if dut.data.value: ...``). (:pr:`4622`)
...
- Prevent multiple Tasks from sharing a :external+cocotb20:meth:`.Lock.acquire` Trigger. If this was shared it would cause all Tasks waiting on that same Trigger to think they have acquired the Lock. (:pr:`4473`)
...
- Fixed a bug where :external+cocotb20:meth:`cancelling <cocotb.task.Task.cancel>` a Task that is awaiting a :external+cocotb20:class:`.First` or :external+cocotb20:class:`.Combine` doesn't cancel the underlying waiter Tasks, leading to lower performance or concealing test failures. (:pr:`4542`)
...
- Support ``waves`` argument to :external+cocotb20:meth:`.Runner.test` for GHDL and NVC. (:pr:`4630`)
...
- Removed ``cocotb.handle.ConstantObject``. Use :external+cocotb20:attr:`.ValueObjectBase.is_const` to determine is an object is immutable instead. (:pr:`2720`)
- Deprecated :class:`int`, :class:`str`, and :class:`float` casts on :external+cocotb20:class:`simulator value objects <cocotb.handle.ValueObjectBase>`. Instead use the :external+cocotb20:attr:`.ValueObjectBase.value` getter, then cast the value. (:pr:`2720`)
...
- The ``prefix`` and ``postfix`` arguments to :external+cocotb20:meth:`.TestFactory.generate_tests` are deprecated in favor of the more flexible ``name`` argument. (:pr:`3578`)
- Methods :external+cocotb19:meth:`~cocotb.simulator.gpi_sim_hdl.get_definition_name()` and :external+cocotb19:meth:`~cocotb.simulator.gpi_sim_hdl.get_definition_file()` of :external+cocotb20:class:`cocotb.handle.SimHandleBase` were removed in favor of :external+cocotb20:meth:`~cocotb.handle.SimHandleBase._def_name` and :external+cocotb20:meth:`~cocotb.handle.SimHandleBase._def_file`, respectively. (:pr:`3609`)
...
- :external+cocotb20:class:`~cocotb.regression.TestFactory` is now deprecated. Use :external+cocotb20:class:`cocotb.parametrize` instead. (:pr:`3717`)
- ``cocotb.handle.ModifiableObject`` was removed along with its non-functional ``drivers()`` and ``loads()`` methods. (:pr:`3733`)
- Removed :external+cocotb20:attr:`cocotb.argc`. Use ``len(cocotb.argv)`` instead. (:pr:`3779`)
- ``cocotb.LANGUAGE`` was removed, use ``os.environ["TOPLEVEL_LANG"]`` if you need that information. (:pr:`3779`)
- :external+cocotb20:attr:`LogicArray.signed_integer <cocotb.types.LogicArray.signed_integer>` has been deprecated. Use :external+cocotb20:meth:`.LogicArray.to_signed` instead. (:pr:`3792`)
...
- :external+cocotb20:attr:`LogicArray.integer <cocotb.types.LogicArray.integer>` has been deprecated. Use :external+cocotb20:meth:`.LogicArray.to_unsigned` instead. (:pr:`3792`)
- The ``cocotb.scheduler`` module and ``cocotb.scheduler`` object have been made private. (:pr:`3806`)
- Deprecated the ``verilog_sources`` and ``vhdl_sources`` parameters to :external+cocotb20:meth:`.Runner.build`. Use the language-agnostic ``sources`` parameter instead. (:pr:`3836`)
- ``Event.fired`` attribute was made private. Use :external+cocotb20:meth:`.Event.is_set`. (:pr:`3851`)
...
- :external+cocotb20:attr:`LogicArray.buff <cocotb.types.LogicArray.buff>` has been deprecated. Use :external+cocotb20:meth:`.LogicArray.to_bytes` instead. (:pr:`3944`)
- The ``outcome`` parameter to :external+cocotb20:class:`~cocotb.triggers.NullTrigger` was removed. There is no alternative. (:pr:`3969`)
- Deprecated the undocumented ``data`` attribute on :external+cocotb20:class:`~cocotb.triggers.Event` and corresponding argument to :external+cocotb20:meth:`.Event.set`. (:pr:`3980`)
- Removed the ``cycles`` argument to :external+cocotb20:meth:`.Clock.start`. Use :external+cocotb20:meth:`.Clock.stop` to stop the Clock at the desired time instead. (:pr:`3983`)
...
- :external+cocotb20:meth:`.Task.join` was deprecated, use the :external+cocotb20:class:`~cocotb.task.Task` being joined directly wherever ``task.join()`` was previously used. (:pr:`4084`)
...
- Constructing a :external+cocotb20:class:`~cocotb.types.LogicArray` from an :class:`int` now only accepts integer literals (unsigned). Use :external+cocotb20:meth:`.LogicArray.from_signed` to convert negative integers into :external+cocotb20:class:`~cocotb.types.LogicArray`\ s using two's complement representation. (:pr:`4093`)
- The module ``cocotb.decorators`` was removed. All functionality is now available directly in the ``cocotb`` namespace (e.g. ``cocotb.decorators.test`` is now :external+cocotb20:func:`cocotb.test`). (:pr:`4129`)
...
- :external+cocotb20:class:`.TestSuccess` was deprecated. Use :external+cocotb20:func:`cocotb.pass_test` instead. (:pr:`4477`)
...
- Removed the undocumented ``__name__`` and ``__qualname__`` attributes from :external+cocotb20:class:`.Task`. (:pr:`4524`)
- ``cocotb.handle.RegionObject`` and ``cocotb.handle.NonHierarchyObject`` have been removed. (:pr:`4541`)
- Deprecated :external+cocotb20:attr:`.Event.name` and passing a ``name`` argument to :external+cocotb20:class:`.Event` constructors. (:pr:`4561`)
- :external+cocotb20:meth:`.Task.kill` is deprecated in favor of :external+cocotb20:meth:`.Task.cancel`. (:pr:`4573`)
- :external+cocotb20:class:`.Range`, :external+cocotb20:class:`.Logic`, :external+cocotb20:class:`.LogicArray`, and :external+cocotb20:class:`.Array` are now only available from the :external+cocotb20:mod:`cocotb.types` module. Their implementation modules are now private. (:pr:`4623`)
- ``cocotb.handle.SimHandle`` was made private. Generally, :external+cocotb20:class:`dut.hierarchy["path/to/thing"] <cocotb.handle.HierarchyObject>` or :external+cocotb20:meth:`.HierarchyObject._get` can be used get objects by a full path, which was the typical public use for this function originally. (:pr:`4623`)
...
- When casting a :external+cocotb20:class:`simulator object <cocotb.handle.SimHandleBase>` to a :class:`str` return the :func:`repr` of the object instead of the :external+cocotb20:meth:`path <cocotb.handle.SimHandleBase._path>`. (:pr:`2720`)
...
- :external+cocotb20:func:`.resume` and :external+cocotb20:func:`.bridge` are now implemented as decorator functions and not types. All attributes, e.g. ``log``, are no longer available. (:pr:`3461`)
...
- Getting a value with :external+cocotb20:attr:`.LogicArrayObject.value` now returns a :external+cocotb20:class:`~cocotb.types.LogicArray` instead of a ``cocotb.binary.BinaryValue``. (:pr:`3634`)
- The module ``cocotb.log`` was renamed to :external+cocotb20:mod:`cocotb.logging` to prevent clashing with :external+cocotb20:attr:`cocotb.log`. (:pr:`3673`)
- Moved ``cocotb.config`` to ``cocotb_tools.config`` and ``cocotb.runner`` to ``cocotb_tools.runner`` to improve startup speed. (:pr:`3731`)
- Getting a value with :external+cocotb20:attr:`.ArrayObject.value` now returns an :external+cocotb20:class:`~cocotb.types.Array` with the appropriate :external+cocotb20:class:`~cocotb.types.Range` set instead of a :class:`list`. This will change how the object is indexed to match the range of the simulation object, instead of ``0`` to ``len(handle)-1`` like before. (:pr:`3733`)
...
- :external+cocotb20:meth:`.Lock.locked` is now a method instead of an attribute. (:pr:`3851`)
...
- ``std_ulogic_vector``/``std_logic_vector`` signals and constants in VHDL and packed arrays of ``logic`` signals and parameters in Verilog are now discovered as :external+cocotb20:class:`.LogicArrayObject`\ s instead of ``cocotb.handle.ModifiableObject``. (:pr:`2720`) (:pr:`4318`)
- ``std_ulogic``/``std_logic`` signals and constants in VHDL and scalar ``logic`` signals and parameters in Verilog are now discovered as :external+cocotb20:class:`.LogicObject`\ s instead of ``cocotb.handle.ModifiableObject``. (:pr:`2720`) (:pr:`4318`)
...
- :external+cocotb20:meth:`.Task.cancel` now causes a :exc:`~asyncio.CancelledError` to be thrown into the coroutine. This behavior is *scheduled*, so Tasks will not become cancelled immediately. (:pr:`4524`)
...
- :external+cocotb20:class:`.Join` was moved from :external+cocotb20:mod:`cocotb.triggers` to :external+cocotb20:mod:`cocotb.task`. (:pr:`4544`)
...
- :external+cocotb20:meth:`.LogicArray.is_resolvable` now returns ``True`` for arrays containing ``H`` and ``L`` values. (:pr:`4551`)
...
- Renamed ``cocotb.types.ArrayLike`` to :external+cocotb20:class:`.AbstractArray`. (:pr:`4623`)
...
- The :external+cocotb18:envvar:`!RTL_LIBRARY`` and :external+cocotb18:envvar:`TOPLEVEL_LIBRARY` Makefile variables were merged into :external+cocotb19:envvar:`TOPLEVEL_LIBRARY`. Update all uses of :envvar:`!RTL_LIBRARY`.
