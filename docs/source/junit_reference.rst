...
This formats aims to support wide range of different CI environments like:
...
* ``hostname`` - name of machine where the testsuite run was executed.
...
.. note::
    ...
    When generating JUnit XML file from pytest, it is possible to include ``file`` and ``line`` as attributes in ``testcase`` elements by setting the pytest ``junit_family`` to ``xunit1``:
