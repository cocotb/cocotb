...
* Replace usage of :external+cocotb19:py:attr:`BinaryValue.integer <cocotb.binary.BinaryValue.integer>` and :external+cocotb19:py:attr:`BinaryValue.signed_integer <cocotb.binary.BinaryValue.signed_integer>`
  with :meth:`.LogicArray.to_unsigned` or :meth:`.LogicArray.to_signed`, respectively.
* Replace usage of :external+cocotb19:py:attr:`BinaryValue.binstr <cocotb.binary.BinaryValue.binstr>` with the :class:`str` cast (this works with :class:`!BinaryValue` as well).
...
.. note::
    This also implies a change to type annotations, if you were using them previously.
...
However, it is possible to construct a :class:`!BinaryValue` without set size.
...
Previously, when modifying a :class:`!BinaryValue` in-place using :meth:`!BinaryValue.assign`
or the :attr:`!BinaryValue.buff`, :attr:`!BinaryValue.binstr`, :attr:`!BinaryValue.integer`, or :attr:`!BinaryValue.signed_integer` setters,
...
:class:`!BinaryValue` could be constructed with a ``binaryRepresentation`` argument
which would select how that :class:`!BinaryValue` would interpret any integer being used to set its value.
:meth:`!BinaryValue.assign` and the :attr:`!BinaryValue.integer` and :attr:`!BinaryValue.signed_integer` setters all behaved the same when given an integer.
